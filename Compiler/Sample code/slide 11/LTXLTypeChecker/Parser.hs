...
happyExpList = Happy_Data_Array.listArray (0,141) ([1536,1202,3072,2404,57344,63,0,0,0,0,0,0,32768,11393,1,22787,2,45574,4,25612,9,0,192,0,0,0,128,0,4096,256,0,0,0,0,0,61440,543,57344,319,0,0,0,0,49152,38464,32768,11393,1,22787,2,45574,4,25612,9,51224,18,36912,37,8288,75,16576,150,0,0,0,0,0,12,0,24,0,60,0,120,0,240,0,508,0,1020,0,0,0,45574,4,0,128,0,192,36912,37,65408,0,0,0,33152,300,64512,263,1536,1202,61440,31,57344,63,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parser","ltxl_program","exp","pexp","decls","decl","type","litint","ident","'||'","'&&'","'<'","'=='","'>'","'+'","'-'","'*'","'/'","'\\\\'","'('","')'","if","then","else","let","in","int","bool","'='","';'","%eof"]
        bit_start = st * 33
        bit_end = (st + 1) * 33
...
{-# LINE 16 "<built-in>" #-}
{-# LINE 1 "/var/folders/9f/dqw5bp2s4k746y2kjlg9_zn80000gn/T/ghc9226_0/ghc_2.h" #-}
