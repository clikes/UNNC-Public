...
-- *									     *
-- *           Example for G53CMP, lectures 11, November 2014		     *
-- *									     *
-- ***************************************************************************

module TypeCheckerFinal(typeCheck, tcAux) where
...
tcAux l env (LitInt n) =
    return TpInt
tcAux l env (Var i) =
    case lookupVar i env of
        Left (_, t) -> return t
        Right m     -> emitErrD NoSrcPos m >> return TpUnknown
tcAux l env (UnOpApp uo e1) = do
    ...
    return t2
tcAux l env (BinOpApp bo e1 e2) = do
    let TpArr (TpProd t1 t2) t3 = lookupBO bo env	-- Safe if comp. corr.
    t1' <- tcAux l env e1
    t2' <- tcAux l env e2
    unless (compatible t1 t1') (emitErrD NoSrcPos (illTypedOpApp t1 t1'))
    unless (compatible t2 t2') (emitErrD NoSrcPos (illTypedOpApp t2 t2'))
    return t3
tcAux l env (If e1 e2 e3) = do
    t1 <- tcAux l env e1
    t2 <- tcAux l env e2
    t3 <- tcAux l env e3
    unless (compatible t1 TpBool) (emitErrD NoSrcPos (illTypedCond t1))
    unless (compatible t2 t3) (emitErrD NoSrcPos (incompatibleBranches t2 t3))
    return t2						-- or t3
tcAux l env (Let ds e) = do
    let l' = l + 1
    env' <- tcDefs l' env ds
    t    <- tcAux l' env' e
    return t
